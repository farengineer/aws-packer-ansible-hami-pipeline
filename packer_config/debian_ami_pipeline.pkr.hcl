source "amazon-ebs" "debian11" {
  ...
  # AMI CONFIG
  ami_name        = "Debian11-${local.timestamp}"
  ami_description = "Debian 11 default image with quartely patching. Built with Packer."
...
build {
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      "echo 'Installing SSM.",
      "mkdir /tmp/ssm",
      "wget https://s3.amazonaws.com/ec2-downloads-windows/SSMAgent/latest/debian_amd64/amazon-ssm-agent.deb -P --directory-prefix=/tmp/ssm",
