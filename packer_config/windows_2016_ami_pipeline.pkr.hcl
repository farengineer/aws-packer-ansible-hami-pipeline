source "amazon-ebs" "ws16" {
  source_ami = data.amazon-ami.ws16.id
  ...
  # AMI CONFIG
  ami_name        = "Windows-Server-2016-${local.timestamp}"
  ami_description = "Windows Server 2016 default image with quartely patching. Built with Packer."
...
build {
  ...
  # Update default TLS options to avoid TLS/SSL secure channel error
  provisioner "powershell" {
    inline = ["[Net.ServicePointManager]::SecurityProtocol = 'Tls, Tls11, Tls12, Ssl3';Invoke-Expression (Invoke-WebRequest -UseBasicParsing -Uri 'https://raw.githubusercontent.com/DarwinJS/Undo-WinRMConfig/master/Undo-WinRMConfig.ps1')"]
  }
