source "amazon-ebs" "al2_cis_1" {
  ...
  # AMI CONFIG
  ami_name        = "Amazon-Linux-2-CIS-1-${local.timestamp}"
  ami_description = "Amazon Linux 2 default image with quartely patching. Built with Packer and Ansible."
...
build {
  name = "amazon_linux_2_cis_1"
  sources = [
    "source.amazon-ebs.al2_cis_1"
  ]

  provisioner "ansible" {
    playbook_file = var.al2_playbook
  }
