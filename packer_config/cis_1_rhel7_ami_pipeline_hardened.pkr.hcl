source "amazon-ebs" "rhel7_cis_1" {
  source_ami = data.amazon-ami.rhel7.id
  ...
  # AMI CONFIG
  ami_name        = "RHEL7-CIS-1-${local.timestamp}"
  ami_description = "RHEL7 default image with quartely patching. Built with Packer."
...
build {
  ...
  provisioner "ansible" {
    playbook_file = var.rhel7_playbook
  }
